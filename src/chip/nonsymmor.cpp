...
NSORChip::NSORChip(int aUS)
{
    ...
    if (aUS)
        mTexture = load_texture("data/mystery_gate.png");
    else
        mTexture = load_texture("data/mystery_gate.png");
...
void NSORChip::render(int aChipId)
{
    if (gBlackBackground)
...
void NSORChip::update(float aTick)
{
    ...
    if ((
            (mInputPinA.mNet->mState == NETSTATE_HIGH || mInputPinA.mNet->mState == NETSTATE_NC) &&
