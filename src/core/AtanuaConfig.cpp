...
void AtanuaConfig::load()
{
    ...
    if (!f)
    {
        ...
        element = new TiXmlElement("DisableRecursiveDeletion");
        topelement->LinkEndChild(element);
        element->SetAttribute("value", mDisableRecursiveDeletion);
                ...
                if (stricmp(root->Value(), "AtanuaConfig")==0)
                {
                    ...
                    for (part = root->FirstChild(); part != 0; part = part->NextSibling())
                    {
                        if (part->Type() == TiXmlNode::ELEMENT)
                        {
                            ...
                            if (stricmp(part->Value(), "DisableRecursiveDeletion") == 0)
                            {
                                ((TiXmlElement*)part)->QueryIntAttribute("value", &mDisableRecursiveDeletion);
                            }
                            else
